  ...
  String? _enableItems(bool check, String? value) {
    if (checkBoxField.items != null && checkBoxField.items!.count > 0) {
      ...
      final PdfDictionary? dic =
          PdfFieldItemHelper.getHelper(
            checkBoxField.items![defaultIndex],
          ).dictionary;
      if (dic != null) {
...
class PdfCheckFieldBaseHelper extends PdfFieldHelper {
  ...
  void save() {
    ...
    if (checkField.form != null) {
      final Map<String, PdfTemplate> checkValue = _createTemplate(
        _checkedTemplate,
      );
      _checkedTemplate = checkValue['template'];
      final Map<String, PdfTemplate> unCheckValue = _createTemplate(
        _uncheckedTemplate,
      );
      _uncheckedTemplate = unCheckValue['template'];
      final Map<String, PdfTemplate> pressedValue = _createTemplate(
        _pressedCheckedTemplate,
      );
      _pressedCheckedTemplate = pressedValue['template'];
      final Map<String, PdfTemplate> unPressedValue = _createTemplate(
        _pressedUncheckedTemplate,
      );
  ...
  void applyAppearance(
    ...
    if (widget != null && item != null) {
      if (item._checkBaseHelper.dictionary!.containsKey(
            PdfDictionaryProperties.v,
          ) &&
      ...
      } else if (!item._checkBaseHelper.dictionary!.containsKey(
            PdfDictionaryProperties.v,
          ) &&
          item is! PdfRadioButtonListItem) {
        widget.remove(PdfDictionaryProperties.v);
    ...
    if ((widget != null) && (widget.containsKey(PdfDictionaryProperties.ap))) {
      ...
      if (appearance.containsKey(PdfDictionaryProperties.n)) {
        ...
        holder = PdfCrossTable.dereference(
...
class PdfRadioButtonListItem extends PdfCheckFieldBase {
  ...
  String get value {
    if (_radioButtonListItemHelper.isLoadedField) {
      _radioButtonListItemHelper._value = _radioButtonListItemHelper
          .getItemValue(
  ...
  @override
  PdfForm? get form =>
      (_radioButtonListItemHelper._field != null)
    ...
    if (_radioButtonListItemHelper.isLoadedField) {
      ...
      if (super.style != value) {
        ...
        WidgetAnnotationHelper.getHelper(
          _radioButtonListItemHelper.widget!,
        ).widgetAppearance!.normalCaption = _radioButtonListItemHelper
            .styleToString(super.style);
      }
    }
  }
}
...
class PdfRadioButtonListItemHelper extends PdfCheckFieldBaseHelper {
  ...
  void _setItemValue(String value) {
    ...
    if (dictionary!.containsKey(PdfDictionaryProperties.ap)) {
      ...
      if (dic.containsKey(PdfDictionaryProperties.n)) {
        final PdfReference normal = crossTable!.getReference(
        ...
        if (dic.containsKey(dicValue)) {
          ...
          dic.setProperty(
            str,
            PdfReferenceHolder.fromReference(valRef, crossTable),
          );
        }
      }
    }
  ...
  void draw() {
    ...
    FieldPainter().drawRadioButton(
      _field!.page!.graphics,
      params,
      styleToString(base.style),
      state,
    );
  }
  ...
  Rect getBounds() {
    ...
    if (array != null && array is PdfArray) {
      ...
      if ((PdfCrossTable.dereference(array[1])! as PdfNumber).value! < 0) {
        y =
            (PdfCrossTable.dereference(array[1])! as PdfNumber).value
                as double?;
